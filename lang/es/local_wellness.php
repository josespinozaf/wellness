<?php
// **Pagina General**
$string ['titulopagina'] = 'Deportes UAI';
$string ['clasesyrutinas'] = 'Clases y Rutinas';

// **Barra Navegacion**

$string ['navpagprinc'] = 'Página Principal';
$string ['navsalapesas'] = 'Rutinas';
$string ['navclases'] = 'Clases';
$string ['navsalapesas'] = 'Sala de Pesas';
$string ['navasistencias'] = 'Mis asistencias';
$string ['naveventos'] = 'Eventos';
$string ['navcontacto'] = 'Contacto';
$string ['navrutinaaleatoria'] = 'Rutina Aleatoria';
$string ['navdepartamento'] = 'Departamento';
$string ['navselecciones'] = 'Selecciones UAI';
$string ['navimc'] = 'IMC UAI';

// **Eventos**

// **Clases**
$string ['titleclases'] = 'Bienvenido a Clases';

// **Sala de Pesas**
$string ['titlesalapesas'] = 'Bienvenido a Sala de Pesas';

// **Rutina Aleatoria**
$string ['rutale'] = 'Rutina Aleatoria';
$string ['ejercicio'] = 'Ejercicio';
$string ['intensidad'] = 'Intensidad';
$string ['tren'] = 'Tren';
$string ['categoria'] = 'Categoria';
$string ['linkvid'] = 'Link Video';
$string ['entrenamiento'] = 'Entrenamiento';
$string ['pesorut'] = 'Peso';
$string ['porcentaje'] = '% Del trabajo';
$string ['repeticiones'] = 'Repeticiones';
$string ['series'] = 'Series';
$string ['descanso'] = 'Descanso entre series';
$string ['resistencia'] = 'Resistencia';
$string ['ligero'] = 'Ligero';
$string ['cinsenpor'] = '50% a 60%';
$string ['doceaveinte'] = '12 a 20';
$string ['tresacuatro'] = '3 a 4';
$string ['veinteatreinta'] = '20 a 30 seg';
$string ['hipertrofia'] = 'Hipertrofia';
$string ['moderado'] = 'Moderado';
$string ['setochpor'] = '70% a 80%';
$string ['ochoadoce'] = '8 a 12';
$string ['treintaanoventa'] = '30 a 90 seg';
$string ['fuerza'] = 'Fuerza';
$string ['pesado'] = 'Pesado';
$string ['ochciepor'] = '80% a 100%';
$string ['dosaseis'] = '2 a 6';
$string ['unoaseis'] = '1 a 6';
$string ['cientovadoscua'] = '120 a 240 seg';
$string ['repnivel'] = 'Las repeticiones son según tu nivel y son de 4 series, ve tu nivel de repeticiones más abajo.';
$string ['volver'] = 'Volver';
$string ['calentamiento']= '*Inicia tu entrenamiento con un calentamiento de 10 min en trotadoras, elípticas, bicicletas, bici-mano, bici-mano-pie';
$string ['core']='*Realiza trabajo core durante 5 min.';
$string ['vueltacalma']='*Vuelta a la calma por 5 min.';
$string ['zona'] = 'Zona muscular';
$string ['rep1'] = 'Rep serie 1';
$string ['rep2'] = 'Rep serie 2';
$string ['rep3'] = 'Rep serie 3';
$string ['rep4'] = 'Rep serie 4';
$string ['rep5'] = 'Rep serie 5';
$string ['tsup'] = 'Tren superior';
$string ['tsupantpos'] = 'Tren superior ant/pos';
$string ['tsupant'] = 'Tren superior anterior';
$string ['tsuppos'] = 'Tren superior posterior';
$string ['tsupinf'] = 'Tren superior/inferior';
$string ['tinf'] = 'Tren inferior';
$string ['tinfantpos'] = 'Tren inferior ant/pos';
$string ['tinfant'] = 'Tren inferior anterior';
$string ['tinfpos'] = 'Tren inferior posterior';
$string ['todocuerpo'] = 'Todo el cuerpo';
$string ['oaerobico'] = 'Aeróbico';
$string ['ocalentamiento'] = 'Calentamiento';
$string ['ocore'] = 'Core';
$string ['ofueres'] = 'Fuerza/Resistencia';
$string ['otgen'] = 'Trabajo General';
$string ['avan'] = 'Avanzado';
$string ['inter'] = 'Intermedio';
$string ['basico'] = 'Básico';


// **Asistencias**
$string['fecha']='Fecha';
$string['horainicio']='Hora Inicio';
$string['horatermino']='Hora Termino';
$string['actividad']='Actividad';
$string['asistencia']='Asistencia';
$string['fafacheck']='<i class="fa fa-check" aria-hidden="true"></i>';
$string['fafacross']='<i class="fa fa-times" aria-hidden="true"></i>';

// **IMC**
$string ['imcs'] = 'IMC UAI';
$string ['ano'] = 'Año';
$string ['formcancel'] = 'El formulario se ha cancelado.';
$string ['estatura'] = 'Estatura (cm)';
$string ['peso'] = 'Peso (Kg)';
$string ['imcm'] = 'IMC';

// **Respuestas**
$string ['elimexito'] = 'Ha sido eliminado con éxito.';
$string ['agrexito'] = 'Ha sido agregado con éxito.';
$string ['erroroc'] = 'Ha ocurrido un error.';
$string ['imgexito'] = 'Se ha ingresado exitosamente la imagen ';
$string ['imgactexito'] = 'Se ha acualizado exitosamente la imagen ';
$string ['imgerror'] = 'No se ha podido actualizar la imagen.';

?>